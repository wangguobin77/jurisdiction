    ...
    public function actionCreate()
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if(!filterData($info['username'],'string',255,1)){
                show_json(100000,'username can not be empty');
            }
            ...
            if($d){
                show_json(100000,'username already exists');//该用户名已存在
            }
            ...
            if($pid = Yii::$app->request->post('pid')){
                ...
                if($AdminUserTier && $AdminUserTier['status'] == 0){//禁用的上级不允许添加子成员
                    show_json(100000,'tier have been banned');//上级被禁用 不能在添加下级
                }
            ...
            if ($AdminUserTier->save()) {
                show_json(0,'add success');
            } else {
                show_json(100000,'add error');
            }
