...
class AdminRuleController extends BaseController
{
        ...
        if (Yii::$app->request->isPost) {
            ...
            if($AdminRuleData['pid'] && $AdminRuleData['pid'] != 0){
                ...
                if(isset($model['is_have_part']) && $model['is_have_part'] == 0){
                    show_json(100000,'Menu permissions cannot be created under this parent class');//该父类下不能创建菜单权限
                }
            }
            ...
            if ($AdminRule->save()) {
                show_json(0,'create rule success');
            }

            show_json(100000,'create rule error');
            ...
            if(Yii::$app->request->get('id')){//如果url有该值 说明是基于父类下创建 如果不存在说明是顶级菜单  添加权限严格控制权限，不能随便赋予用户这个权限
                ...
                if(!$model){
                    show_json(100000,'Data information does not exist');
                }
            }
    ...
    public function actionUpdate()
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            Yii::$app->response->format = Response::FORMAT_JSON;
            if ($model->save()) {
                show_json(0,'update success');
            }

            show_json(100000,'update error');
        } else {
    ...
    public function actionDel()
    {
        ...
        if(count($p_data) > 0){
            show_json(100000,'The current routing has a subset Can\'t delete');//当前路由存在自己所以不能删除
        }
        ...
        if (AdminRule::findOne($id)->delete()) {
           /* $url = Url::toRoute('admin-rule/index');
            header('location:'.$url);
            die;*/
            show_json(0,'del success');
        }

        show_json(100000,'del error');
    }
